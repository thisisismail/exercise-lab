...
export default class SpotifyAPIFirstStep extends Component {
  ...
  getDataAPI = async (v,x) => { // deklarasi fungsi asinkron untuk mengambil promise dari fetch
    if(v === ''){ return 0 }else{this.setState({message: 'NOW YOU ARE AUTHORIZED'}); this.setState({login: 'RELOGIN'})}
    if(x === ''){ return 0 }
    const token = v;
    const input = x;
  ...
  submitInput(){
    this.setState(() => this.getDataAPI(this.state.getauthapi, this.state.input));
  ...
  render() {
    const {apidata, message, login} = this.state;
    ...
    return (
      <div>
        <div>{message}</div>
        <a href={AUTH_URL}><button onClick={this.handleBtnAuth} href={AUTH_URL}>{login}</button></a>
        <div>
